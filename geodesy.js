...
module.exports = function geodesy(element) {
    ...
    let motifData = require('./motif/' + motif + '.json')['motif'];
    let normData = {};
    let sortedNormData = [];
    // for(var shape of motifData){
    motifData.forEach((shape, motifIndex) => {
        let basispts = applyShift(M(dot(table(size, size), shape.basis)), shape.offset);
        console.log({
            polygon: motifIndex,
            basispts
        });
...
function polygon2clippath(polygon) {
    ...
    */
    return [
        'polygon(',
        polygon.map(pt => pt.map(n => N(λ.run(`(${n} + 1) * 50`)) + '%').join(' ')).join(','),
        ')'
    ].join('');
}
