...
var elementary = require('@lookalive/elementary').elementary;
...
// /www/
var cache = {};
fs.readdirSync('./cache').map(function (filename) {
...
http.createServer(function (req, res) {
    var _a;
    var _b = url.parse(req.url).pathname.split('/').slice(1), route = _b[0], resource = _b[1], subresource = _b.slice(2);
    if (!route) {
        _a = ['cgi', 'index'], route = _a[0], resource = _a[1];
    }
    try {
        switch (route) {
            case "static":
                ...
                break;
            case "cgi":
                var html = zlib.gzipSync(elementary(requireNoCache('../cgi/' + resource + '.js')));
                res.writeHead(200, {
                ...
                break;
            case "cache":
                var json = zlib.gzipSync(JSON.stringify(keypath(cache[resource], subresource))); // you'll get null back if the key doesn't exist within that resource
                res.writeHead(200, {
